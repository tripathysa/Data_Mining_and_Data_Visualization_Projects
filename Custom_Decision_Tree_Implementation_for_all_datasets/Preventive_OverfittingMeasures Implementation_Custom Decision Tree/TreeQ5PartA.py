...
class treebuilder:
    ...
    def calcError(self,attributes, dataset, classAttributename):
        Frequency = {}
        Listerrors=[]
        i = 0
        ginival = 0.0
        ...
        for item in dataset:
            if (Frequency.has_key(item[i])):
                ...
                Frequency[item[i]]  = 1.0
        for freq in Frequency.values():
            Listerrors.append((freq/len(dataset)))
        return 1-max(Listerrors)
    ...
    def getMostLikelyAttribute(self,dataset, attributes, classAttributename):
        ...
        for attr in attributes:
            ...
            if newGain>maxGain:
                maxGain = newGain
                best = attr
        error=self.calcError(attributes, dataset, classAttributename)
        pessimisticerror=error+((self.leveltraverse)/(2*len(dataset)))
        if self.errorslistperIteration:
            if pessimisticerror<self.errorslistperIteration[len(self.errorslistperIteration)-1]:
               self.errorslistperIteration.append(pessimisticerror)
               return best
            else:
                return None
        return best
    ...
    def buildDecisionTree(self,dataset, attributes, classAttributename, countrecursion):
        countrecursion += 1
        dataset = dataset[:]
        ...
        if not dataset or (len(attributes) - 1) <= 0:
            ...
            for val in self.getcolumnValues(dataset, attributes, best):
                ...
                newAttr = attributes[:]
                newAttr.remove(best)
                subtree = self.buildDecisionTree(examples, newAttr, classAttributename, countrecursion)
